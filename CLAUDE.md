...
- `evalHand(cards)` — NLHE: SF≥8e8, Quads≥7e8, **FH≥6e8**, **Flush≥5e8**, Straight≥4e8, Trips≥3e8, TwoPair≥2e8, Pair≥1e8
- `evalHandSD(cards)` / `eval5SD(cards)` — Short Deck: same thresholds except **Flush≥6e8** (above FH) and **FH≥5e8** (below Flush); A-6-7-8-9 is a valid straight (`strHi=9`)
...
4. `.hand-chart` — the `activeGridN×activeGridN` grid (37px × 47px cells); `gridTemplateColumns/Rows` set dynamically in `buildChart()`
